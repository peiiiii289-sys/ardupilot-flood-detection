...
const AP_Scheduler::Task Plane::scheduler_tasks[] = {
    ...
    SCHED_TASK(update_ai_vision_timeout, 10, 100, 49),
...
bool Plane::send_ai_landing_start_auto()
{
    mavlink_command_long_t cmd{};

    cmd.target_system = 1;
    cmd.target_component = 211;
    cmd.command = MAV_CMD_USER_1;
    cmd.confirmation = 0;

    cmd.param1 = 1.0f;   // stream enable
    cmd.param2 = 10.0f;  // freq hz
    cmd.param3 = MAV_FRAME_BODY_FRD;
    ...
    gcs().send_text(MAV_SEVERITY_INFO,
                    "AI_AUTO_START_CMD sent");

    return true;
}

bool Plane::send_ai_landing_stop_auto()
{
    mavlink_command_long_t cmd{};
    ...
    gcs().send_text(MAV_SEVERITY_INFO,
                    "AI_AUTO_STOP_CMD sent");

    return true;
}

bool Plane::send_ai_vision_start(float mode, float stream, float freq_hz)
{
    mavlink_command_long_t cmd{};

    cmd.target_system = 1;
    cmd.target_component = 211;
    cmd.command = MAV_CMD_USER_3;
    cmd.confirmation = 0;
    ...
    landing_ai.vision_ack_start_wait(MAV_CMD_USER_3, AP_HAL::millis());

    gcs().send_text(MAV_SEVERITY_INFO,
                    "AI_VISION_START_CMD sent mode=%.0f stream=%.0f freq=%.1f",
                    (double)mode,
                    (double)stream,
                    (double)freq_hz);
...
bool Plane::send_ai_vision_stop()
{
    mavlink_command_long_t cmd{};

    cmd.target_system = 1;
    cmd.target_component = 211;
    cmd.command = MAV_CMD_USER_4;
    cmd.confirmation = 0;
    ...
    landing_ai.vision_ack_start_wait(MAV_CMD_USER_4, AP_HAL::millis());

    gcs().send_text(MAV_SEVERITY_INFO,
                    "AI_VISION_STOP_CMD sent");

    return true;
}

void Plane::check_ai_vision_ack_timeout(uint32_t now_ms)
{
    if (!landing_ai.vision_ack_timed_out(now_ms, 1000)) {
        return;
    }

    if (landing_ai.vision_timeout_reported()) {
        return;
    }

    const uint16_t cmd = landing_ai.vision_ack_command();
    landing_ai.vision_ack_mark_timeout();

    gcs().send_text(MAV_SEVERITY_ERROR,
                    "AI_VISION_ACK_TIMEOUT cmd=%u timeout_ms=1000",
                    (unsigned)cmd);
}
...
/*
  Try to automatically start the AI landing stream.

  Story 2.5 requirements:
  - Prefer AGL.
  - Fall back to altitude relative to Home when AGL is unavailable.
  - Trigger only when altitude is at or below AILND_STRT_ALT.
  - Send only once per landing cycle.
 */
bool Plane::try_ai_landing_auto_start(const char *source)
{
    if (landing_ai.auto_start_sent()) {
        return false;
    }

    float trigger_alt_m = 0.0f;
    bool used_agl = false;

    const float agl_m =
        relative_ground_altitude(RangeFinderUse::TAKEOFF_LANDING);

    if (isfinite(agl_m) && agl_m >= 0.0f) {
        trigger_alt_m = agl_m;
        used_agl = true;
    } else {
        float position_d_home_m = 0.0f;

        // This API returns void in the current ArduPilot version.
        // It internally provides a Home-relative altitude fallback.
        ahrs.get_relative_position_D_home(position_d_home_m);

        // NED D is positive downward.
        trigger_alt_m = -position_d_home_m;

        if (!isfinite(trigger_alt_m)) {
            return false;
        }
    }

    if (trigger_alt_m > g.ailnd_strt_alt.get()) {
        return false;
    }

    if (!send_ai_landing_start_auto()) {
        gcs().send_text(
            MAV_SEVERITY_WARNING,
            "AI_AUTO_START_FAILED source=%s",
            source != nullptr ? source : "UNKNOWN");
        return false;
    }

    landing_ai.mark_auto_start_sent();

    gcs().send_text(
        MAV_SEVERITY_INFO,
        "AI_AUTO_START source=%s alt=%.2f src=%s hz=%d",
        source != nullptr ? source : "UNKNOWN",
        (double)trigger_alt_m,
        used_agl ? "AGL" : "REL_HOME",
        (int)g.ailnd_start_hz.get());
