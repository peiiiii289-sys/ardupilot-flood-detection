...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    ...
    case MAV_CMD_USER_3: { // 31012 = START AI VISION
        plane.landing_ai.set_ai_vision_enabled(true);

        if (!plane.send_ai_vision_start(packet.param1, packet.param2, packet.param3)) {
            gcs().send_text(MAV_SEVERITY_ERROR,
                            "AI_VISION_START send failed");
            return MAV_RESULT_FAILED;
        }
    ...
    case MAV_CMD_USER_4: { // 31013 = STOP AI VISION
        plane.landing_ai.set_ai_vision_enabled(false);

        if (!plane.send_ai_vision_stop()) {
            gcs().send_text(MAV_SEVERITY_ERROR,
                            "AI_VISION_STOP send failed");
            return MAV_RESULT_FAILED;
        }
...
void GCS_MAVLINK_Plane::handle_message(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_AI_LANDING_CORRECTION: {
        mavlink_ai_landing_correction_t pkt;
        mavlink_msg_ai_landing_correction_decode(&msg, &pkt);

        plane.landing_ai.set_camera_offset(
        plane.g.ail_cam_x.get(),
        plane.g.ail_cam_y.get(),
        plane.g.ail_cam_z.get());

        plane.landing_ai.update_correction(
            pkt.roll_err,
            pkt.pitch_err,
            pkt.yaw_err,
            pkt.x_err,
        ...
        // bit0 = yaw_valid
        // bit1 = distance_valid
        const bool yaw_valid = (pkt.flags & 0x01U) != 0;
        const bool distance_valid = (pkt.flags & 0x02U) != 0;

        if (!yaw_valid || !distance_valid) {
            gcs().send_text(
                MAV_SEVERITY_WARNING,
                "AI_LANDING_CORR_INVALID flags=%u yaw_valid=%u distance_valid=%u",
                (unsigned)pkt.flags,
                (unsigned)yaw_valid,
                (unsigned)distance_valid
            );
        }

        gcs().send_text(
            MAV_SEVERITY_INFO,
            "AI_CORR conf=%.2f dist=%.2f",
    ...
    case MAVLINK_MSG_ID_AI_LANDING_STATUS: {
        mavlink_ai_landing_status_t pkt;
        mavlink_msg_ai_landing_status_decode(&msg, &pkt);

        plane.landing_ai.update_status(
            pkt.visual_confidence,
            pkt.target_lost,
            pkt.reproj_error,
            pkt.covariance,
            AP_HAL::millis()
        ...
        plane.landing_ai.evaluate_levels(now_ms);

        gcs().send_text(MAV_SEVERITY_INFO,
                        "AI_STAT conf=%.2f lost=%u reproj=%.2f cov=%.2f L1=%u L2=%u L3=%u reason=%u",
                        (double)pkt.visual_confidence,
                        (unsigned)pkt.target_lost,
                        (double)pkt.reproj_error,
                        (double)pkt.covariance,
                        (unsigned)plane.landing_ai.l1_warning(),
                        (unsigned)plane.landing_ai.l2_go_around(),
                        (unsigned)plane.landing_ai.l3_takeover(),
                        (unsigned)plane.landing_ai.level_reason());

        static bool l1_announced = false;
        static bool l3_announced = false;

        if (plane.landing_ai.l1_active_long_enough(now_ms, 1000)) {
            if (!l1_announced) {
                gcs().send_text(MAV_SEVERITY_WARNING,
                                "AI_LANDING_L1_WARNING vzmax=0.30 damp=1.20 reason=%u",
                                (unsigned)plane.landing_ai.level_reason());
                l1_announced = true;
            }
        } else {
        ...
        if (plane.landing_ai.l3_takeover()) {
            if (!l3_announced) {
                gcs().send_text(MAV_SEVERITY_CRITICAL,
                                "AI_LANDING_TAKEOVER reason=%u",
                                (unsigned)plane.landing_ai.level_reason());
                l3_announced = true;
            }
        } else {
    ...
    case MAVLINK_MSG_ID_COMMAND_ACK: {
        ...
        if (pkt.command == MAV_CMD_USER_3 || pkt.command == MAV_CMD_USER_4) {
            const bool matched = plane.landing_ai.vision_ack_handle(
                pkt.command,
                pkt.result,
                pkt.result_param2
            );
            ...
            if (matched) {
                gcs().send_text(MAV_SEVERITY_INFO,
                                "AI_VISION_ACK cmd=%u result=%u param2=%u",
                                (unsigned)pkt.command,
                                (unsigned)pkt.result,
                                (unsigned)pkt.result_param2);
            } else {
                gcs().send_text(MAV_SEVERITY_WARNING,
                                "AI_VISION_ACK_UNMATCHED cmd=%u result=%u param2=%u",
                                (unsigned)pkt.command,
                                (unsigned)pkt.result,
                                (unsigned)pkt.result_param2);
            }
