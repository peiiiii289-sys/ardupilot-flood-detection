...
void ModeQLoiter::run()
{
    ...
    static uint32_t last_qloiter_enter_log = 0;
    if (now - last_qloiter_enter_log > 1000) {
        gcs().send_text(MAV_SEVERITY_INFO, "QLOITER ENTER");
        last_qloiter_enter_log = now;
    }
    ...
    */
    ...
    if (quadplane.throttle_wait) {
        static uint32_t last_qloiter_throttle_wait_log = 0;
        if (now - last_qloiter_throttle_wait_log > 1000) {
            gcs().send_text(MAV_SEVERITY_INFO, "QLOITER RETURN throttle_wait");
            last_qloiter_throttle_wait_log = now;
        }
    ...
    static uint32_t last_qloiter_control_log = 0;
    if (now - last_qloiter_control_log > 1000) {
        gcs().send_text(MAV_SEVERITY_INFO, "QLOITER CONTROL REACHED");
        last_qloiter_control_log = now;
    }
    ...
    // ===== AI Landing Injection (QLoiter level) =====
    if (plane.control_mode == &plane.mode_qland) {

    const bool ai_corr_ok =
        plane.landing_ai.correction_valid(now, 1000, 0.5f);

    if (ai_corr_ok &&
        !plane.landing_ai.target_lost() &&
        plane.landing_ai.confidence() > 0.5f) {

            const float ai_pitch_rad =
            ...
            float ai_gain = 0.2f;
            if (height_agl_m <= 2.0f) {
                ai_gain = 0.8f;
            } else if (height_agl_m < 10.0f) {
                ai_gain = 0.8f - ((height_agl_m - 2.0f) / 8.0f) * 0.6f;
                // 2m -> 0.8, 10m -> 0.2
            }

            const float ai_pitch_inject_raw_cd = ai_pitch_cd * ai_gain;

            // L1 damping: when L1 warning is active long enough, reduce injection aggressiveness
            float l1_scale = 1.0f;
            if (plane.landing_ai.l1_active_long_enough(now, 1000)) {
                l1_scale = 1.0f / plane.landing_ai.l1_damp();   // 1 / 1.2 = 0.833...
            }

            const float ai_pitch_inject_l1_cd = ai_pitch_inject_raw_cd * l1_scale;

            // simple low-pass filter for smoother injection
            static float ai_pitch_inject_filtered_cd = 0.0f;
            ai_pitch_inject_filtered_cd =
                0.8f * ai_pitch_inject_filtered_cd + 0.2f * ai_pitch_inject_l1_cd;

            plane.nav_pitch_cd += ai_pitch_inject_filtered_cd;

            static uint32_t last_ai_inject_log = 0;
            if (now - last_ai_inject_log > 1000) {
                gcs().send_text(MAV_SEVERITY_INFO,
                    "AI_INJECT_QLOITER raw=%.1f l1=%.1f filt=%.1f gain=%.2f h=%.1f conf=%.2f",
                    (double)ai_pitch_inject_raw_cd,
                    (double)ai_pitch_inject_l1_cd,
                    (double)ai_pitch_inject_filtered_cd,
                    (double)ai_gain,
                    (double)height_agl_m,
                    (double)plane.landing_ai.confidence());
                last_ai_inject_log = now;
            }
        } else {
            static uint32_t last_ai_fallback_log = 0;
            if (now - last_ai_fallback_log > 1000) {
                gcs().send_text(MAV_SEVERITY_WARNING,
                                "AI_CORR_FALLBACK recent=%u corr_conf=%.2f status_conf=%.2f",
                                (unsigned)plane.landing_ai.correction_recent(now, 1000),
                                (double)plane.landing_ai.correction_confidence(),
                                (double)plane.landing_ai.confidence());
                last_ai_fallback_log = now;
            }
        }
    }
