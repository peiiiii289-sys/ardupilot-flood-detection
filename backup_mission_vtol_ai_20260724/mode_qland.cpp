...
void ModeQLand::run()
{
    ...
    // Story 2.5: 正式版 START（AGL + fallback + param）
    float trigger_alt_m = 0.0f;
    bool used_agl = false;
    bool have_trigger_alt = false;
    ...
    if (isfinite(agl_m) && agl_m >= 0.0f) {
        trigger_alt_m = agl_m;
        used_agl = true;
        have_trigger_alt = true;
    } else {
        // fallback: relative home altitude
        float posD = 0.0f;
        plane.ahrs.get_relative_position_D_home(posD);
        trigger_alt_m = -posD;   // NED down -> altitude above home
        used_agl = false;
        have_trigger_alt = isfinite(trigger_alt_m);
    }

    if (plane.arming.is_armed() &&
        !plane.landing_ai.auto_start_sent() &&
        have_trigger_alt &&
        trigger_alt_m <= plane.g.ailnd_strt_alt) {

        plane.send_ai_landing_start_auto();

        gcs().send_text(MAV_SEVERITY_INFO,
                        "AI_AUTO_START trigger alt=%.2f src=%s hz=%d",
                        (double)trigger_alt_m,
                        used_agl ? "AGL" : "REL_HOME",
                        (int)plane.g.ailnd_start_hz.get());

        plane.landing_ai.mark_auto_start_sent();
    }
